...
io.on('connection', (socket) => {
  ...
  socket.on('send-message', (data) => {
    ...
    socket.emit('receive-message', {
      ...
      senderId: socket.id,
      timestamp: new Date(),
      isOwnMessage: true, // Flag to indicate this is user's own message
    ...
    socket.to(roomId).emit('receive-message', {
      ...
      senderId: socket.id,
      timestamp: new Date(),
      isOwnMessage: false, // Flag to indicate this is partner's message
      shouldSpeak: true,   // Speak partner's messages
      // You can also pass the original message if needed for translation
      originalMessage: message
    });

    console.log(`✅ Message sent to room ${roomId}. Own message: shown without speech. Partner message: with speech.`);
  });
  ...
  socket.on('translation-request', async (data) => {
    const { roomId, text, sourceLang, targetLang, isForSpeech = false } = data;
    console.log('🔄 OpenAI Translation request:', { text, sourceLang, targetLang, isForSpeech });
    ...
    try {
      ...
      socket.emit('translation-result', {
        ...
        sourceLang,
        targetLang,
        isForSpeech // Pass this flag to indicate if this is for speech synthesis
      });
  ...
  // New event for text-to-speech requests (for speaking translated text)
  socket.on('speak-translated-text', (data) => {
    const { roomId, text, language, isOwnMessage = false } = data;
    const room = rooms.get(roomId);
    ...
    if (!isOwnMessage) {
      console.log(`🔊 Request to speak text: "${text}" in ${language} for user ${socket.id}`);
  ...
  // New event to trigger speech synthesis on partner's side
  socket.on('trigger-partner-speech', (data) => {
    const { roomId, text, language, targetUserId } = data;
    const room = rooms.get(roomId);

    if (!room || !room.users.has(socket.id) || !room.users.has(targetUserId)) {
      return;
    }
    ...
    console.log(`🔊 Triggered speech for partner ${targetUserId}: "${text}" in ${language}`);
  });
...
function fallbackTranslation(text, sourceLang, targetLang) {
  const translations = {
    'hello': { es: 'hola', fr: 'bonjour', de: 'hallo', hi: 'नमस्ते', ja: 'こんにちは', zh: '你好', ko: '안녕하세요', ar: 'مرحبا', pt: 'olá', ru: 'привет' },
    'thank you': { es: 'gracias', fr: 'merci', de: 'danke', hi: 'धन्यवाद', ja: 'ありがとう', zh: '谢谢', ko: '감사합니다', ar: 'شكرا', pt: 'obrigado', ru: 'спасибо' },
    'goodbye': { es: 'adiós', fr: 'au revoir', de: 'auf wiedersehen', hi: 'अलविदा', ja: 'さようなら', zh: '再见', ko: '안녕히 가세요', ar: 'مع السلامة', pt: 'adeus', ru: 'до свидания' },
    'please': { es: 'por favor', fr: 's\'il vous plaît', de: 'bitte', hi: 'कृपया', ja: 'お願いします', zh: '请', ko: '제발', ar: 'من فضلك', pt: 'por favor', ru: 'пожалуйста' },
    'yes': { es: 'sí', fr: 'oui', de: 'ja', hi: 'हाँ', ja: 'はい', zh: '是的', ko: '예', ar: 'نعم', pt: 'sim', ru: 'да' },
    'no': { es: 'no', fr: 'non', de: 'nein', hi: 'नहीं', ja: 'いいえ', zh: '不', ko: '아니요', ar: 'لا', pt: 'não', ru: 'нет' },
    'how are you': { es: '¿cómo estás?', fr: 'comment ça va?', de: 'wie geht es dir?', hi: 'आप कैसे हैं?', ja: 'お元気ですか？', zh: '你好吗？', ko: '어떻게 지내세요?', ar: 'كيف حالك؟', pt: 'como você está?', ru: 'как дела?' },
    'what is your name': { es: '¿cómo te llamas?', fr: 'comment tu t\'appelles?', de: 'wie heißt du?', hi: 'आपका नाम क्या है?', ja: 'お名前は何ですか？', zh: '你叫什么名字？', ko: '당신의 이름은 무엇입니까?', ar: 'ما اسمك؟', pt: 'qual é o seu nome?', ru: 'как тебя зовут?' },
    'good morning': { es: 'buenos días', fr: 'bonjour', de: 'guten morgen', hi: 'शुभ प्रभात', ja: 'おはようございます', zh: '早上好', ko: '좋은 아침', ar: 'صباح الخير', pt: 'bom dia', ru: 'доброе утро' },
    'good night': { es: 'buenas noches', fr: 'bonne nuit', de: 'gute nacht', hi: 'शुभ रात्रि', ja: 'おやすみなさい', zh: '晚安', ko: '안녕히 주무세요', ar: 'تصبح على خير', pt: 'boa noite', ru: 'спокойной ночи' }
  };
